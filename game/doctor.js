(function () {
  ...
  doctor.draw = function (ctx, t) {
    ...
    if( this.dead ) {
      ...
      ctx.fillRect(this.loc.x - 6, this.loc.y + 3, size.hand, size.hand);
      
      ctx.fillRect(this.loc.x + 20, this.loc.y + 3, size.hand, size.hand);

      // Feet
      ctx.fillStyle = "hsla(0,0%,70%,1)";
      ctx.fillRect(this.loc.x + 5, this.loc.y + 0.5 * Math.sin(t / 10) + 4, 10, 6);

      // Body shadow
      ctx.fillStyle = "hsla(0,0%,90%,1)";
      ctx.fillRect(this.loc.x, this.loc.y - 3, 20, 10);
      
      // Body
      ctx.fillStyle = "hsla(0,0%,100%,1)";
      ctx.fillRect(this.loc.x, this.loc.y, 20, 10);
